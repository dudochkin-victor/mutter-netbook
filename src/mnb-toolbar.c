 ...
 mnb_toolbar_show_completed_cb (ClutterAnimation *animation, ClutterActor *actor)
{
 ...
 mnb_toolbar_show_lowlight (MnbToolbar *toolbar)
 {
	 ...
	 clutter_actor_save_easing_state (lowlight);
	 clutter_actor_set_easing_duration (lowlight, TOOLBAR_LOWLIGHT_FADE_DURATION);
	 clutter_actor_set_easing_mode(lowlight, CLUTTER_EASE_IN_SINE);
	 clutter_actor_set_opacity(lowlight, 0x7f);
	 clutter_actor_restore_easing_state (lowlight);
 ...
 mnb_toolbar_check_for_windows (MnbToolbar *toolbar)
 {
	 ...
	 while (l)
	 {
		 ...
		 if (!(type == META_WINDOW_DOCK    ||
				 ...
				 meta_window_is_override_redirect (meta_window_actor_get_meta_window(m))))
		 {
...
mnb_toolbar_show_pending_panel (MnbToolbar *toolbar, MnbToolbarPanel *tp)
{
	...
	meta_screen_get_size (meta_plugin_get_screen(priv->plugin),
 ...
 */
 ...
 mnb_toolbar_panel_show_begin_cb (MnbPanel *panel, MnbToolbar *toolbar)
 {
	 ...
	 if (CLUTTER_ACTOR_IS_VISIBLE (priv->panel_stub))
	 {
		 ...
		 clutter_actor_save_easing_state (priv->panel_stub);
		 clutter_actor_set_easing_duration (priv->panel_stub, SLIDE_DURATION);
		 clutter_actor_set_easing_mode(priv->panel_stub, CLUTTER_EASE_IN_SINE);
		 clutter_actor_set_width(priv->panel_stub, wf);
		 clutter_actor_set_height(priv->panel_stub, hf);
		 clutter_actor_set_opacity(priv->panel_stub, 0);
		 clutter_actor_restore_easing_state (priv->panel_stub);
	 }
		...
		MnbToolbarPanel *tp)
{
	...
	clutter_actor_add_child (CLUTTER_ACTOR (priv->hbox),
			CLUTTER_ACTOR (panel));
 ...
 */
		 ...
		 gboolean         panel_destroyed)
 {
	 ...
	 if (!panel_destroyed && CLUTTER_IS_ACTOR (panel))
		 clutter_actor_remove_child (CLUTTER_ACTOR (priv->hbox),
				 CLUTTER_ACTOR (panel));
 }
 ...
 mnb_toolbar_constructed (GObject *self)
 {
	 ...
	 ClutterActor      *shadow;
	 ClutterTexture    *sh_texture = NULL;
	 gint               screen_width, screen_height;
	 ...
	 meta_screen_get_size (meta_plugin_get_screen(plugin),
			 &priv->old_screen_width,
			 &priv->old_screen_height);

	 clutter_actor_set_reactive (actor, TRUE);

	 hbox = priv->hbox = clutter_actor_new ();
	 ...
	 clutter_actor_add_child (CLUTTER_ACTOR (wgroup), lowlight);
...

	 {
		 ...
		 clutter_actor_add_child (CLUTTER_ACTOR (wgroup), panel_stub);
	 ...
	 if (sh_texture)
	 {
		 ...
		 clutter_actor_add_child (CLUTTER_ACTOR (wgroup), shadow);
	 ...
	 if (netbook_mode)
		 priv->trigger_cb_id =
				 g_signal_connect (meta_get_stage_for_screen(meta_plugin_get_screen (META_PLUGIN (plugin))),
						 "captured-event",
						 G_CALLBACK (mnb_toolbar_stage_captured_cb),
						 self);

	 g_signal_connect (meta_get_stage_for_screen(meta_plugin_get_screen (plugin)),
			 "button-press-event",
 ...
 mnb_toolbar_ensure_size_for_screen (MnbToolbar *toolbar)
 {
	 ...
	 meta_screen_get_size (meta_plugin_get_screen(priv->plugin), &screen_width, &screen_height);
	 ...
	 {
		 priv->trigger_cb_id =
				 g_signal_connect (meta_get_stage_for_screen(meta_plugin_get_screen (priv->plugin)),
						 "captured-event",
						 G_CALLBACK (mnb_toolbar_stage_captured_cb),
						 toolbar);
	 }
	 ...
	 {
		 g_signal_handler_disconnect (meta_get_stage_for_screen(meta_plugin_get_screen (priv->plugin)),
				 priv->trigger_cb_id);
 ...
 mnb_toolbar_stage_show_cb (ClutterActor *stage, MnbToolbar *toolbar)
 {
	 ...
	 stg    = CLUTTER_STAGE (meta_get_stage_for_screen(meta_plugin_get_screen (plugin)));
