...
static const GDebugKey options_keys[] = { { "compositor-always-on",
		MNB_OPTION_COMPOSITOR_ALWAYS_ON }, { "disable-ws-clamp",
		MNB_OPTION_DISABLE_WS_CLAMP }, { "disable-panel-restart",
		MNB_OPTION_DISABLE_PANEL_RESTART }, { "composite-fullscreen-apps",
		MNB_OPTION_COMPOSITE_FULLSCREEN_APPS }, };
...
static void meta_window_fullscreen_notify_cb(GObject *object, GParamSpec *spec,
		gpointer data);
...
static void maximize(MetaPlugin *plugin, MetaWindowActor *actor, gint x, gint y,
		gint width, gint height);
static void unmaximize(MetaPlugin *plugin, MetaWindowActor *actor, gint x,
...
get_actor_private(MetaWindowActor *actor) {
	...
	if (G_UNLIKELY (!priv)) {
		...
		g_object_set_qdata_full(G_OBJECT (actor), actor_data_quark, priv,
				free_actor_private);
	}
...
static void meego_netbook_plugin_dispose(GObject *object) {
	g_warning("meego_netbook_plugin_dispose");
...
static void meego_netbook_plugin_finalize(GObject *object) {
	g_warning("meego_netbook_plugin_finalize");
	mnb_input_manager_destroy();

	G_OBJECT_CLASS (meego_netbook_plugin_parent_class) ->finalize(object);
}

static void meego_netbook_plugin_set_property(GObject *object, guint prop_id,
...
static void meego_netbook_plugin_get_property(GObject *object, guint prop_id,
 ...
 */
static void meego_netbook_workarea_changed_cb(MetaScreen *screen,
		MetaPlugin *plugin) {
	g_warning("meego_netbook_workarea_changed_cb");
...
static void meego_netbook_overlay_key_cb(MetaDisplay *display,
		MetaPlugin *plugin) {
	g_warning("meego_netbook_overlay_key_cb");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
...
static gboolean meego_netbook_fullscreen_apps_present_on_workspace(
		MetaPlugin *plugin, gint index) {
	g_warning("meego_netbook_fullscreen_apps_present_on_workspace");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	GList *l;
...
static void meego_netbook_workspace_switched_cb(MetaScreen *screen, gint from,
		gint to, MetaMotionDirection dir, MetaPlugin *plugin) {
	g_warning("meego_netbook_fullscreen_apps_present_on_workspace");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	gboolean on;
...
static void meego_netbook_compute_screen_size(Display *xdpy, gint screen_no,
		gint *width_mm, gint *height_mm, gboolean *external) {
	g_warning("meego_netbook_compute_screen_size");
...
static void meego_netbook_panel_window_show_cb(MetaWindowActor *mcw,
		MetaPlugin *plugin) {
	g_warning("meego_netbook_panel_window_show_cb");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	MnbToolbar *toolbar = MNB_TOOLBAR (priv->toolbar);
	MnbPanel *panel;
	Window xwin = meta_window_get_xwindow(
			meta_window_actor_get_meta_window(mcw));
	...
	g_signal_handlers_disconnect_by_func(mcw,
			meego_netbook_panel_window_show_cb, plugin);
}

static void meego_netbook_display_window_created_cb(MetaDisplay *display,
		MetaWindow *win, MetaPlugin *plugin) {
	g_warning("meego_netbook_display_window_created_cb");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	MnbToolbar *toolbar = MNB_TOOLBAR (priv->toolbar);
	MetaWindowActor *mcw;
	MetaWindowType type;
	...
	if (type == META_WINDOW_DOCK) {
		...
		if ((panel = mnb_toolbar_find_panel_for_xid(toolbar, xwin))) {
			 ...
			 */
			//g_object_set (mcw, "shadow-type", MUTTER_SHADOW_ALWAYS, NULL); //DV
			g_signal_connect(mcw, "show",
					G_CALLBACK (meego_netbook_panel_window_show_cb), plugin);
	...
	if (meta_window_is_override_redirect(
		...
		if (wm_class && !strcmp(wm_class, "Gnome-screensaver")) {
			...
			g_signal_connect(mcw, "window-destroyed",
					G_CALLBACK (window_destroyed_cb), plugin);
		}
	}
...
static void meego_netbook_display_focus_window_notify_cb(MetaDisplay *display,
		GParamSpec *spec, MetaPlugin *plugin) {
	g_warning("meego_netbook_display_focus_window_notify_cb");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	...
	if (mw && priv->last_focused != mw) {
		 ...
		 */
		if (type != META_WINDOW_DOCK && xwin != priv->focus_xwin
			...
			if (mw)
				g_object_weak_ref(G_OBJECT (mw), last_focus_weak_notify_cb,
						plugin);
		}
	}
}
...
static void meego_netbook_handle_screen_size(MetaPlugin *plugin,
		gint *screen_width, gint *screen_height) {
	g_warning("meego_netbook_handle_screen_size");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	...
	g_warning("meego_netbook_handle_screen_size 2!!!!!!!");
	/*
	 ...
	 */
	meta_screen_get_size(screen, screen_width,
			screen_height);

	if (old_screen_width == *screen_width
			&& old_screen_height == *screen_height)
		return;
	g_warning("meego_netbook_handle_screen_size 2.5!!!!!!!");
	old_screen_width = *screen_width;
	old_screen_height = *screen_height;

	force_small_screen = gconf_client_get_bool(priv->gconf_client,
			KEY_ALLWAYS_SMALL_SCREEN, NULL );

	if (!force_small_screen) {
		meego_netbook_compute_screen_size(xdpy, screen_no, &screen_width_mm,
				&screen_height_mm, &external);

		g_debug("Screen size %dmm x %dmm, external %d", screen_width_mm,
				screen_height_mm, external);
	}
	...
	g_warning("meego_netbook_handle_screen_size 3!!!!!!!");
	leader_xwin = meta_display_get_leader_window(display);

	meego_session = g_strdup_printf("session-type=%s",
			netbook_mode ? "small-screen" : "bigger-screen");

	g_debug("Setting _MEEGO=%s", meego_session);

	meta_error_trap_push(display);
	XChangeProperty(xdpy, leader_xwin, atom__MEEGO, XA_STRING, 8,
			PropModeReplace, (unsigned char*) meego_session,
			strlen(meego_session));
	meta_error_trap_pop(display);

	g_free(meego_session);

	gconf_client_set_string(priv->gconf_client, KEY_THEME,
			netbook_mode ? "Netbook" : "Nettop", NULL );

	gconf_client_set_string(priv->gconf_client, KEY_BUTTONS,
			netbook_mode ? ":close" : ":maximize,close", NULL );
	g_warning("meego_netbook_handle_screen_size 4!!!!!!!");
	/*
	...
	if (!netbook_mode && CLUTTER_ACTOR_IS_VISIBLE (stage)
			&& !CLUTTER_ACTOR_IS_VISIBLE (toolbar)) {
		g_warning("try to show toolbar");
		mnb_toolbar_show(toolbar, MNB_SHOW_HIDE_POLICY);
	} else if (netbook_mode && CLUTTER_ACTOR_IS_VISIBLE (stage)
			&& CLUTTER_ACTOR_IS_VISIBLE (toolbar)
			&& !mnb_toolbar_get_active_panel(toolbar)) {

		g_warning("try to hide toolbar");
		...
		if (XQueryPointer(xdpy, xwin, &root, &child, &root_x, &root_y, &x, &y,
				&mask)) {
	...
	if (toolbar)
		g_warning("toolbar is exist");
	else
		g_warning("toolbar does not exist");
	g_warning("meego_netbook_handle_screen_size end");
}

static void meego_netbook_plugin_start(MetaPlugin *plugin) {
	g_warning("meego_netbook_plugin_start");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	...
	gconf_client_set_string(gconf_client,
			"/apps/metacity/global_keybindings/cycle_group", "disabled", NULL );
	gconf_client_set_string(gconf_client,
	...
	mx_texture_cache_get_texture (mx_texture_cache_get_default (),
			THEMEDIR
			"/panel/panel-shadow.png");
	g_warning ("LOAD CACHE %s", THEMEDIR "/mx.cache");
...
//                                 THEMEDIR "/mx.cache");
	...
	g_signal_connect(screen, "workareas-changed",
			G_CALLBACK (meego_netbook_workarea_changed_cb), plugin);

	g_signal_connect(screen, "workspace-switched",
			G_CALLBACK (meego_netbook_workspace_switched_cb), plugin);

	g_signal_connect(display, "overlay-key",
			G_CALLBACK (meego_netbook_overlay_key_cb), plugin);

	g_signal_connect(display, "window-created",
			G_CALLBACK (meego_netbook_display_window_created_cb), plugin);

	g_signal_connect(display, "notify::focus-window",
			G_CALLBACK (meego_netbook_display_focus_window_notify_cb), plugin);

	overlay = meta_get_overlay_group_for_screen(meta_plugin_get_screen(plugin));
	 ...
	 */
	priv->toolbar = CLUTTER_ACTOR (mnb_toolbar_new (plugin));
	 ...
	 */
	clutter_actor_add_child(CLUTTER_ACTOR (overlay), priv->toolbar);
	clutter_actor_add_child(CLUTTER_ACTOR (overlay), switcher_overlay);

	clutter_actor_add_child(CLUTTER_ACTOR (stage), message_overlay);
	...
	mnb_toolbar_show(priv->toolbar, MNB_SHOW_HIDE_POLICY);

	g_warning("HERE");
	/* Keys */

	//meta_prefs_override_no_tab_popup (TRUE); //DV
}

/*static MutterShadow *
 meego_netbook_get_shadow (MetaPlugin *plugin, MetaWindowActor *mcw)
 {
...
static void meego_netbook_plugin_class_init(MeegoNetbookPluginClass *klass) {
	g_warning("meego_netbook_plugin_class_init");
...
static void meego_netbook_plugin_init(MeegoNetbookPlugin *self) {
	g_warning("meego_netbook_plugin_init");
	...
	if ((options = g_getenv("MUTTER_COMPOSITOR_OPTIONS"))) {
		...
		compositor_options = g_parse_debug_string(options, options_keys,
 ...
 */
static void on_minimize_effect_complete(ClutterTimeline *timeline,
		EffectCompleteData *data) {
	g_warning("on_minimize_effect_complete");
	/*
	...
	ActorPrivate *apriv;
	MetaWindowActor *mcw = META_WINDOW (data->actor);
 ...
 * completion).
 */
static void minimize(MetaPlugin * plugin, MetaWindowActor *mcw)

{
	...
	if (type == META_WINDOW_NORMAL) {
		...
		animation = clutter_actor_animate(actor, CLUTTER_EASE_IN_SINE,
				MINIMIZE_TIMEOUT, "scale-x", 0.0, "scale-y", 0.0, NULL );

		data->actor = actor;
		data->plugin = plugin;

		g_signal_connect(clutter_animation_get_timeline (animation),
				"completed", G_CALLBACK (on_minimize_effect_complete), data);
	} else
		meta_plugin_minimize_completed(plugin, mcw);
}
 ...
 * calls the manager callback function.
 */
static void on_maximize_effect_complete(ClutterTimeline *timeline,
		EffectCompleteData *data) {
	g_warning("on_maximize_effect_complete");
	/*
 ...
 */
static void maximize(MetaPlugin *plugin, MetaWindowActor *mcw, gint end_x,
		gint end_y, gint end_width, gint end_height) {
	g_warning("maximize");
	ClutterActor *actor = CLUTTER_ACTOR (mcw);
	MetaWindowType type;
	...
	if (type == META_WINDOW_NORMAL) {
		...
		clutter_actor_set_pivot_point(actor, anchor_x, anchor_y);

		animation = clutter_actor_animate(actor, CLUTTER_EASE_IN_SINE,
				MAXIMIZE_TIMEOUT, "scale-x", scale_x, "scale-y", scale_y,
				NULL );
		...
		g_signal_connect(clutter_animation_get_timeline (animation),
				"completed", G_CALLBACK (on_maximize_effect_complete), data);
		return;
	}
 ...
 */
static void unmaximize(MetaPlugin *plugin, MetaWindowActor *mcw, gint end_x,
		gint end_y, gint end_width, gint end_height) {
	g_warning("unmaximize");
	MetaWindowType type;
...
static void meego_netbook_move_window_to_workspace(MetaWindowActor *mcw,
		gint workspace_index, guint32 timestamp) {
	g_warning("meego_netbook_move_window_to_workspace");
...
static void meego_netbook_move_window_to_its_workspace(MetaPlugin *plugin,
		MetaWindowActor *mcw, guint32 timestamp) {
	g_warning("meego_netbook_move_window_to_its_workspace");
	MetaScreen *screen = meta_plugin_get_screen(plugin);
	gint index;
	gint n_workspaces;
	...
	if (n_workspaces == 1) {
		...
		while (l) {
			...
			if (m != mcw && (type == META_WINDOW_NORMAL)
					&& !meta_window_is_hidden(mw)
					&& !meta_window_is_override_redirect(mw)
...
static void on_map_effect_complete(ClutterTimeline *timeline,
		EffectCompleteData *data) {
	g_warning("on_map_effect_complete");
	/*
	 * Must reverse the effect of the effect.
	 */
	MetaPlugin *plugin = data->plugin;
	MetaWindowActor *mcw = META_WINDOW_ACTOR (data->actor);
	ActorPrivate *apriv = get_actor_private(mcw);
 ...
 * as a one-of check.
 */
static gboolean maybe_show_myzone(MetaPlugin *plugin) {
	g_warning("maybe_show_myzone");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	MetaScreen *screen = meta_plugin_get_screen(plugin);
	...
	while (l) {
		MetaWindowActor *m = l->data;
		MetaWindowType type = meta_window_get_window_type(
				meta_window_actor_get_meta_window(m));
		...
		if (!(type == META_WINDOW_DESKTOP || type == META_WINDOW_DOCK
			...
			/*          meta_window_get_description (m), type); */
	...
	if (no_apps)
		mnb_toolbar_activate_panel(MNB_TOOLBAR (priv->toolbar),
				"meego-panel-myzone", MNB_SHOW_HIDE_POLICY);

	return FALSE;
}

static void check_for_empty_workspace(MetaPlugin *plugin, gint workspace,
		MetaWindow *ignore, gboolean win_destroyed) {
	g_warning("check_for_empty_workspace");
	MetaScreen *screen = meta_plugin_get_screen(plugin);
	gboolean workspace_empty = TRUE;
	GList *l;
	Window xwin = None;
	...
	while (l) {
		 ...
		 */
		if (mw != ignore && !meta_window_is_on_all_workspaces(mw)
				&& ((win_destroyed && xwin != xt)
						|| (!win_destroyed
								&& !meta_window_is_ancestor_of_transient(ignore,
										mw)))) {
			/* g_debug ("querying workspace for [%s]", */
			/*          meta_window_get_description (m)); */

			gint w = meta_window_get_workspace(
					meta_window_actor_get_meta_window(m));
	...
	if (workspace_empty) {
		...
		if (active_ws == current_ws) {
			...
			if (next_index != -1) {
				...
				next_ws = meta_screen_get_workspace_by_index(screen,
						next_index);
 ...
 */
static gboolean check_for_windows_of_wm_class_and_name(MetaPlugin *plugin,
		const gchar *wm_class, const gchar *wm_name, MetaWindowActor *ignore) {
	g_warning("check_for_windows_of_wm_class_and_name");
	MetaScreen *screen = meta_plugin_get_screen(plugin);
	GList *l;
...
static void handle_window_destruction(MetaWindowActor *mcw, MetaPlugin *plugin) {
	g_warning("handle_window_destruction");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	MetaWindowType type;
	gint workspace;
	MetaWindow *meta_win;
	...
	g_signal_handlers_disconnect_by_func(meta_win,
			meta_window_fullscreen_notify_cb, plugin);

	g_signal_handlers_disconnect_by_func(mcw, window_destroyed_cb, plugin);
	...
	if (type == META_WINDOW_NORMAL || type == META_WINDOW_DIALOG
		 ...
		 */
		if (wm_class && wm_name && !strcmp(wm_class, "Skype")
				&& strstr(wm_name, "Skype™")
				&& !check_for_windows_of_wm_class_and_name(plugin, "Skype",
						"Skype™", mcw)) {
...
static void meta_window_workspace_changed_cb(MetaWindow *mw, gint old_workspace,
		gpointer data) {
	g_warning("meta_window_workspace_changed_cb");
...
static void fullscreen_app_added(MetaPlugin *plugin, MetaWindow *mw) {
	g_warning("fullscreen_app_added");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
...
static void fullscreen_app_removed(MetaPlugin *plugin, MetaWindow *mw) {
	g_warning("fullscreen_app_removed");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	gboolean compositor_on;
...
gboolean meego_netbook_fullscreen_apps_present(MetaPlugin *plugin) {
	g_warning("meego_netbook_fullscreen_apps_present");
	MetaScreen *screen = meta_plugin_get_screen(plugin);
	gint active;
...
static void meego_netbook_detach_mutter_windows(MetaScreen *screen) {
	g_warning("meego_netbook_detach_mutter_windows");
...
static void meego_netbook_toggle_compositor(MetaPlugin *plugin, gboolean on) {
	g_warning("meego_netbook_toggle_compositor");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	MetaScreen *screen;
	Display *xdpy;
	Window xroot;
	...
	if ((priv->compositor_disabled && !on)
			|| (!priv->compositor_disabled && on))
		return;

	screen = meta_plugin_get_screen(plugin);
	xdpy = meta_display_get_xdisplay(meta_screen_get_display(screen));
	...
	if (!on) {
		...
		XCompositeUnredirectSubwindows(xdpy, xroot, CompositeRedirectManual);
		...
		XCompositeRedirectSubwindows(xdpy, xroot, CompositeRedirectManual);
		XSync(xdpy, FALSE);
...
static void meta_window_fullscreen_notify_cb(GObject *object, GParamSpec *spec,
		gpointer data) {
	g_warning("meta_window_fullscreen_notify_cb");
 ...
 */
static gboolean meta_window_is_blessed_window(MetaWindow *mw) {
	g_warning("meta_window_is_blessed_window");
	const gchar *wm_class;
...
static gboolean meego_netbook_window_is_modal_for_panel(MnbPanelOop *panel,
		MetaWindow *window) {
	g_warning("meego_netbook_window_is_modal_for_panel");
	MetaWindowActor *mcw;
	MetaWindow *mw;
	 ...
	 * we need to.
	 */
	if (!meta_window_is_modal(window) || !(trans_for =
			meta_window_get_transient_for(window))) {
		return FALSE;
	}
...
static void meego_netbook_panel_modal_destroyed_cb(MetaWindowActor *mcw,
		MnbPanelOop *panel) {
	g_warning("meego_netbook_panel_modal_destroyed_cb");
	mnb_panel_oop_set_auto_modal(panel, FALSE);
}

static gboolean meego_netbook_never_move_window(MetaWindow *mw) {
	g_warning("meego_netbook_never_move_window");
 ...
 * completion).
 */
static void map(MetaPlugin *plugin, MetaWindowActor *mcw) {
	g_warning("map");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	MnbToolbar *toolbar = MNB_TOOLBAR (priv->toolbar);
	ClutterActor *actor = CLUTTER_ACTOR (mcw);
	...
	g_signal_connect(mw, "notify::fullscreen",
			G_CALLBACK (meta_window_fullscreen_notify_cb), plugin);
	 ...
	 * to NORMAL
	 */
	if (meta_window_is_override_redirect(mw)) {
	 ...
	 */
	else if (type == META_WINDOW_NORMAL || type == META_WINDOW_SPLASHSCREEN
			|| type == META_WINDOW_DIALOG || type == META_WINDOW_MODAL_DIALOG) {
		ClutterAnimation *animation;
		ActorPrivate *apriv = get_actor_private(mcw);
		MetaScreen *screen = meta_plugin_get_screen(plugin);
		gint screen_width, screen_height;
		...
		if (priv->netbook_mode) {
			MnbThreeState state = meego_netbook_mutter_hints_on_new_workspace(
					mw);

			if (state == MNB_STATE_YES) {
				move_window = TRUE;
			} else if (state == MNB_STATE_UNSET) {
				move_window = (type == META_WINDOW_NORMAL
		...
		if (move_window) {
			...
			meego_netbook_move_window_to_its_workspace(plugin, mcw, timestamp);
		}

		/*
		 * Anything that we do not animated exits at this point.
		 */
		if (type == META_WINDOW_DIALOG || type == META_WINDOW_MODAL_DIALOG
				|| fullscreen) {
			meta_plugin_map_completed(plugin, mcw);
			return;
		}
		...
		g_signal_connect(mw, "workspace-changed",
				G_CALLBACK (meta_window_workspace_changed_cb), plugin);

		/*
		 * Only animate windows that are smaller than the screen size
		 * (see MB#5273)
		 */
		meta_screen_get_size(meta_plugin_get_screen(plugin), &screen_width,
				&screen_height);
		...
		if ((gint) actor_width < screen_width
			...
			animation = clutter_actor_animate(actor, CLUTTER_EASE_OUT_ELASTIC,
					MAP_TIMEOUT, "scale-x", 1.0, "scale-y", 1.0, NULL );
			data->plugin = plugin;
			data->actor = actor;
			apriv->tml_map = clutter_animation_get_timeline(animation);

			g_signal_connect(apriv->tml_map, "completed",
					G_CALLBACK (on_map_effect_complete), data);
		} else
			meta_plugin_map_completed(plugin, mcw);
	} else {
		/*
...
static void destroy(MetaPlugin *plugin, MetaWindowActor *mcw) {
	g_warning("destroy");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	MetaWindowType type;
	Window xwin;
	...
	type = meta_window_get_window_type(meta_window_actor_get_meta_window(mcw));
	xwin = meta_window_get_xwindow(meta_window_actor_get_meta_window(mcw));
...
static void last_focus_weak_notify_cb(gpointer data, GObject *meta_win) {
	g_warning("last_focus_weak_notify_cb");
...
static void kill_window_effects(MetaPlugin *plugin, MetaWindowActor *mcw) {
	g_warning("kill_window_effects");
	ActorPrivate *apriv;
...
static void setup_focus_window(MetaPlugin *plugin) {
	g_warning("setup_focus_window");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	Window xwin;
	XSetWindowAttributes attr;
	MetaScreen *screen = meta_plugin_get_screen(plugin);
	Display *xdpy = meta_display_get_xdisplay(meta_screen_get_display(screen));
	MetaDisplay *display = meta_screen_get_display(screen);
#if 0
	Atom type_atom;
...
#if 0
// GdkRegion *mutter_window_get_obscured_region (MetaWindowActor *cw); //DV
gboolean mutter_window_effect_in_progress (MetaWindowActor *self);
...
mnb_get_background_visible_region (MetaScreen *screen)
{
	...
	for (; l; l = l->next)
	{
		...
		if (!META_IS_WINDOW (l->data) || !CLUTTER_ACTOR_IS_VISIBLE (l->data))
		continue;
 ...
 *
 */
static gboolean mnb_desktop_texture_paint(ClutterActor *actor,
		gboolean complete, MetaScreen *screen) {
	g_warning("mnb_desktop_texture_paint");
 ...
 */
static void desktop_background_paint(ClutterActor *background,
		MetaPlugin *plugin) {
	g_warning("desktop_background_paint");
	MetaScreen *screen;
	...
	if (!mnb_desktop_texture_paint(background,
			clutter_actor_is_in_clone_paint(background), screen)) {
		/*
	...
	finish_up: g_signal_stop_emission_by_name(background, "paint");
}

static void setup_desktop_background(MetaPlugin *plugin, const gchar *filename) {
	g_warning("setup_desktop_background %s", filename);
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	MetaScreen *screen = meta_plugin_get_screen(plugin);
	gint screen_width, screen_height;
	...
	meta_screen_get_size(meta_plugin_get_screen(META_PLUGIN (plugin) ),
			&screen_width, &screen_height);

	g_assert(filename);

	new_texture = (ClutterActor*) mx_texture_cache_get_texture(
			mx_texture_cache_get_default(), filename);
	...
	if (!new_texture) {
		...
		if (new_texture) {
			clutter_actor_add_child(CLUTTER_ACTOR (stage), new_texture);
			clutter_actor_lower_bottom(new_texture);

			g_signal_connect(new_texture, "paint",
					G_CALLBACK (desktop_background_paint), plugin);
		}
	}
}

static void desktop_background_changed_cb(GConfClient *client, guint cnxn_id,
		GConfEntry *entry, gpointer data) {
	g_warning("desktop_background_changed_cb");
	MetaPlugin *plugin = META_PLUGIN (data);
	...
	if (!strcmp(key, KEY_BG_FILENAME)) {
		...
		if (clr_str && *clr_str && clutter_color_from_string(&clr, clr_str)) {
			MetaScreen *screen = meta_plugin_get_screen(plugin);
			ClutterActor *stage = meta_get_stage_for_screen(screen);

			clutter_actor_set_background_color(CLUTTER_ACTOR (stage), &clr);
		}
	}
#if 0
...
static void desktop_background_init(MetaPlugin *plugin) {
	g_warning("desktop_background_init");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	GError *error = NULL;

	gconf_client_add_dir(priv->gconf_client, BG_KEY_DIR,
	...
	gconf_client_notify_add(priv->gconf_client, BG_KEY_DIR,
			desktop_background_changed_cb, plugin, NULL, &error);
...
static void focus_xwin(MetaPlugin *plugin, guint xid) {
	g_warning("focus_xwin");
	MetaDisplay *display;
	...
	XSetInputFocus(meta_display_get_xdisplay(display), xid, RevertToPointerRoot,
			CurrentTime);

	meta_error_trap_pop(display);
}

void meego_netbook_stash_window_focus(MetaPlugin *plugin, guint32 timestamp) {
	g_warning("meego_netbook_stash_window_focus");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
...
void meego_netbook_unstash_window_focus(MetaPlugin *plugin, guint32 timestamp) {
	g_warning("meego_netbook_unstash_window_focus");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;
	MetaScreen *screen = meta_plugin_get_screen(plugin);
	MetaDisplay *display = meta_screen_get_display(screen);
	MetaWindow *focus;
 ...
 * workspace.
 */
gboolean meego_netbook_modal_windows_present(MetaPlugin *plugin, gint workspace) {
	g_warning("meego_netbook_modal_windows_present");
	MetaScreen *screen = meta_plugin_get_screen(plugin);
	GList *l = meta_get_window_actors(screen);
	...
	while (l) {
		...
		meta_window_get_transient_for_as_xid (w) == None) {if (workspace < 0)
		{
			return TRUE;
		}
		else
		{
			gint s = meta_window_get_workspace (meta_window_actor_get_meta_window(m));
...
gboolean meego_netbook_compositor_disabled(MetaPlugin *plugin) {
	g_warning("meego_netbook_compositor_disabled");
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;

	return priv->compositor_disabled;
}

void meego_netbook_activate_window(MetaWindow *window) {
	g_warning("meego_netbook_activate_window");
	MetaWorkspace *workspace;
	MetaWorkspace *active_workspace;
	MetaScreen *screen;
	MetaDisplay *display;
...
gboolean meego_netbook_activate_mutter_window(MetaWindowActor *mcw) {
	g_warning("meego_netbook_activate_mutter_window");
	MetaWindow *window;
	MetaWorkspace *workspace;
	MetaWorkspace *active_workspace;
	MetaScreen *screen;
...
gboolean meego_netbook_use_netbook_mode(MetaPlugin *plugin) {
	MeegoNetbookPluginPrivate *priv = MEEGO_NETBOOK_PLUGIN (plugin) ->priv;

	return priv->netbook_mode;
}

guint32 meego_netbook_get_compositor_option_flags(void) {
	return compositor_options;
}

gboolean meego_netbook_urgent_notification_present(MetaPlugin *plugin) {
 ...
 */
void meego_netbook_set_struts(MetaPlugin *plugin, gint left, gint right,
		gint top, gint bottom) {
	g_warning("meego_netbook_set_struts");
